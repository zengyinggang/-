import requests
from bs4 import BeautifulSoup
import base64
import execjs
import re

relation_dict ={
    "data:image/png;base64,":0,
    "data:image/png;base64,":1,
    "data:image/png;base64,":2,
    "data:image/png;base64,":3,
    "data:image/png;base64,": 4,
    "data:image/png;base64,": 5,
    "data:image/png;base64,":6,
    "data:image/png;base64,":7,
    "data:image/png;base64,":8,
    "data:image/png;base64,":9,
}

raw_header = """
Accept: application/json, text/javascript, */*; q=0.01
Accept-Encoding: gzip, deflate
Accept-Language: zh-CN,zh;q=0.9
Cache-Control: no-cache
Connection: keep-alive
Cookie: Hm_lvt_c99546cf032aaa5a679230de9a95c7db=1625465874; Hm_lvt_9bcbda9cbf86757998a2339a0437208e=1625465874; Hm_lpvt_9bcbda9cbf86757998a2339a0437208e=1625465874; qpfccr=true; no-alert3=true; Hm_lpvt_c99546cf032aaa5a679230de9a95c7db=1625466818
Host: match.yuanrenxue.com
Pragma: no-cache
Referer: http://match.yuanrenxue.com/match/4
User-Agent: yuanrenxue.project 
X-Requested-With: XMLHttpRequest
"""

my_headers = {i.split(":", maxsplit=1)[0].strip(): i.split(":", maxsplit=1)[1].strip() for i in raw_header.split("\n") if i != ''}

url = "http://match.yuanrenxue.com/api/match/4"

def get_key_value(number):
    resp = requests.get(url, headers=my_headers, params={"page": number})
    value = resp.json()
    return value

def b64_encode(value: str):
    if isinstance(value, str):
        encode_value = base64.b64encode(bytes(value, "utf-8"))
        return encode_value.decode("utf-8").replace("=", "")
    else:
        raise ValueError("检查value类型,当前类型是%s" % type(value))

def js_md5_encode(value, default_path = "./04.js", mode="r"):
    with open(default_path, mode) as f:
        js_file = f.read()
    js_env = execjs.get()
    ctx = js_env.compile(js_file)
    result = ctx.call("hex_md5", value)
    return result

def main(page_number):
    value = get_key_value(page_number)

    concat_value = value["key"] + value["value"]

    value_tds = value["info"]

    soup = BeautifulSoup(value_tds, "lxml")
    tds_list = soup.find_all("td")

    # 计算b64编码
    b64_value = b64_encode(concat_value)

    # 计算本页md5值
    md5 = js_md5_encode(b64_value)

    # 每个td下面的所有imgs
    page_value = []
    td_value = []

    for td in tds_list:
        soup = BeautifulSoup(str(td), "lxml")
        imgs_list = soup.find_all("img")
        for img in imgs_list:
            # 数字关系的key
            result = re.search(r"src=\"(.*)\"\s", str(img))
            result_md5 = re.search(r"img_number\s(.*)\"\ssrc=\"", str(img))
            result_style = re.search(r"style=\"(.*)\"", str(img))
            key = result.group(1)
            relation_number = relation_dict[key]
            if result_md5.group(1) != md5:
                td_value.append((relation_number, result_style.group(1)))
        # page_value.append(td_value.copy())
        # td_value.clear()
        # 组合每个td里面的数字
        counter = 1
        td_item = list() 
        for order in td_value.copy():
            order_number = float(re.search(r"left:(.*)px", order[1]).group(1))
            if order_number != 0:
                number = order[0]
                number_order = counter + order_number / 11.5
            else:
                number = order[0]
                number_order = counter
            counter += 1
            td_item_tuple = (number, number_order)
            td_item.append(td_item_tuple)
        td_value.clear()

        td_item.sort(key=lambda x: x[1])
        number = ""
        for per_number in td_item:
            number += str(per_number[0])
        page_value.append(int(number))

    return page_value

total_number = []
for i in range(1, 6):
    v = main(i)
    total_number += v
t = 0
for i in total_number:
    t += i
print(t)
